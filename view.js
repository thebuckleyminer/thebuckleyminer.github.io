...
function makeTable(horizontal = 10, vertical = 15) {
    let tableVariable = document.getElementById("assignment3Table");
...
function generateWordsForGameState() {
    let counter = 0
...
function generateCardObjectTypeForGameState() {
...
/*
function flipCardForPlayerRole() {
    let counter = 0
    let  cardCounter= 'card_'+counter
    for (i = 1; i<=25; i++) {
        counter += 1;
        cardCounter= 'card_'+counter;
        if (gameState.otherInfo.showBackColorSide === true) {
            gameState[cardCounter].currentSide = "back"
        } else if (gameState.otherInfo.showBackColorSide === false) {
            gameState[cardCounter].currentSide = "front"
        }
    }    
}
...
function reRenderCards() {
    ...
    for (var i = 1; i<26; i++) {
        ...
        if (gameState[cardCounter].currentSide === "front") {
            reRender.classList = ('tan square');
        } else if (gameState[cardCounter].currentSide === "back") {
            reRender.classList = (gameState[cardCounter].cardObjectType+" square");
        }
    }
}
*/
...
generateWordsForGameState()
generateCardObjectTypeForGameState()
//flipCardForPlayerRole()
...
function constructTable(horizontal, vertical) {
    ...
    for (i = horizontal; i > 0; i--) {
        ...
        for (j = vertical; j > 0; j--) {
            ...
            cardCounter= 'card_'+counter;
            if (gameState[cardCounter].currentSide === "front") {
                htmlOutput += "<td class=\"" + 'tan' + " square\" id=\""+cardCounter+"\"><div class=\"wordbox\">" + gameState[cardCounter].tileWord + "</div></td>";
            } else if (gameState[cardCounter].currentSide === "back") {
                htmlOutput += "<td class=\"" + gameState[cardCounter].cardObjectType + " square\" id=\""+cardCounter+"\"><div class=\"wordbox\">" + gameState[cardCounter].tileWord + "</div></td>";
            }                    
        }
        htmlOutput += "</tr>";
    }
